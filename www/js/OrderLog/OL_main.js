...
document.addEventListener('show', function(event){
  if(event.target.matches('#PM_main')){
    loadTable("PM_table","Goods","");
  }else if(event.target.matches('#OD_main')){
    loadTable("OD_table","Galley");
  }else if(event.target.matches('#OL_main')){
    loadTable("OL_table","OrderLog");
  }else{
  }
},false);
...
/* 注文表示画面のテーブル作成 */
function OL_makeTable(List){
  var old_value="";
  var tableEle = document.getElementById('OL_table');
  var goodsObIDList = new Array(List.length);
  for(i=0;i < List.length; i++){
    goodsObIDList[i] = List[i][1];
  }
